...
void task(Tree* node) {//функция преобразования
    ...
    task(node->left);//рекурсия для потомков
    task(node->right);

    if (node->value.type == symb_OP && node->value.data.op == OP_MULTIPLY) {
        Tree* leftTree = node->left;
        Tree* rightTree = node->right;
        if (leftTree->value.type == symb_NUMBER && rightTree->value.type != symb_NUMBER) {
            int count = (int)leftTree->value.data.number;
            node->value.data.op = OP_PLUS;
            free(node->left);
            node->left = NULL;
            if(count == 1) {//случай умножения на 1
                node->value.data = rightTree->value.data;
                node->value.type = rightTree->value.type;
                free(node->right);
                node->right = NULL;
                return;
            }
            Tree* start = treeCopy(node);
            for (int i = 1; i < count - 1; i++) {//копирование части дерева
                node->left = treeCopy(start);
                node = node->left;
            }
            clearTree(start);
            Tree* last = treeCopy(rightTree);
            node->left = last;
        } else if (rightTree->value.type == symb_NUMBER) {
            int count = (int)rightTree->value.data.number;
            node->value.data.op = OP_PLUS;
            free(node->right);
            node->right = NULL;
            if(count == 1) {//случай умножения на 1
                node->value.data = leftTree->value.data;
                node->value.type = leftTree->value.type;
                free(node->left);
                node->left = NULL;
                return;
            }
            Tree* start = treeCopy(node);
            for (int i = 1; i < count - 1; i++) {//копирование части дерева
                node->right = treeCopy(start);
                node = node->right;
                
            }
            clearTree(start);
            Tree* last = treeCopy(leftTree);
            node->right = last;
        }
    }
}
