...
typedef struct abiturient {
    char surname[MAXSIZE]; // фамилия
    char initials[MAXSIZE]; // инициалы
    char gender[MAXSIZE]; // пол
    int schoolNum; // номер школы
    char medal[MAXSIZE]; // наличие золотой медали
    char composition[MAXSIZE]; // наличие зачета по сочинению
    int numExams; // количество экзаменов
    node* examStats; // список экзаменов
} abiturient;
...
//функции абитуриента и функции абитуриента
...
//функции чтения из файла(текстовый и бинарный)
