...
void printMenu() {
    ...
    printf("| task - найти абитуриентов-медалистов, не набравших проходной балл\n");
...
void task(HashTable* table, float passBall) {
    for(int i = 0; i < TABLESIZE; i++) {
        if (table->entries[i].value != NULL) {
            ...
            if (strcmp(tmp->medal, "Yes") == 0) { //проверка на наличие медали
                ...
                if (balls < passBall) { // если балл меньше проходного, то печатаем его
...
int main(int argc, char *argv[]) {
    ...
    while(strcmp(choose, "exit") != 0) { 
        ...
        if (strcmp(cmd, "add") == 0) {  // добавление в файл
            ...
            printAbiturientChars(find); // если нашли, то печатем о нем информацию
        } else if (strcmp(cmd, "table") == 0) { // вывод таблицы абитуриентов
            printAbiturientStr(table);
        } else if (strcmp(cmd, "task") == 0) { // задание
            printf("Введите проходной балл: ");
            scanf("%f", &ball);
            getchar(); //чтобы не вылетало после выполнения task(программа обрабатывает число, потому считает его в цикле за команду)
            task(table, ball);
        } else { 
            continue;
        }
